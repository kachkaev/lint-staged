 ...
 * @param {boolean} [options.noBackup] - Disable the backup stash, and do not revert in case of errors
...
const runAll = async (
  ...
  if (noBackup) {
    console.warn(`
  ${symbols.warning} ${chalk.yellow('Skipping backup because `--no-backup` was used.')}
`)
  }
...
`)
  ...
  const runner = new Listr(
    [
      {
        title: 'Preparing...',
        task: ctx => git.prepare(ctx, noBackup)
      },
      {
      ...
      ...listrTasks,
      {
        ...
        skip: ctx => !noBackup && shouldSkipApplyModifications(ctx)
      },
      {
